// @zakkster/lite-logn -- demo static file server (repo-only dev artifact, NEVER shipped).
//
//   npm run demo:serve            (then open http://localhost:8018/)
//   node demo/serve.mjs [port]
//
// A zero-dependency Node http server (node:http + node:fs only). Its web root is the LiteLogN
// REPO ROOT, so the page at /demo/index.html and its `../LogN.js` + `./kernels.mjs` imports all
// resolve from one origin. There is no dynamic route -- lite-logn's demo drives the REAL shipped
// classes in the browser directly (no Node-side workload to hand over), so this server only
// serves static files and fails CLOSED on every bad path (null -> 404). Throw prefix:
// [lite-logn-demo].
//
...
/** The default port. Overridable via `node demo/serve.mjs [port]`. Distinct from lite-o1's 8020. */
...
export async function handle(req, res) {
    ...
    // (../LogN.js, ./kernels.mjs) would resolve wrong. Redirecting makes the browser re-request
