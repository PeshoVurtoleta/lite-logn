/**
 * @zakkster/lite-logn -- benchmark orchestrator.
 *
 ...
 *
 * Children are spawned WITH --expose-gc so the D6 allocation-rate curve is real;
 * the parent needs no special flags.
 */
...
/** The cheap comparable scalar the sentinel compares across two runs (D2 first checkpoint). */
export function sentinelMetric(result) {
    if (result && Array.isArray(result.points) && result.points.length > 0) return result.points[0].nsPerOp;
    return 0;
}
...
async function runCell() {
    ...
    // Quiesce before the two THERMAL-SENSITIVE dimensions -- the D1 witness fit (its
    // shallow sub-4ns lines tilt under scheduler/thermal residue) and the D6 heap-delta
    // alloc curve (its ~1 B/op noise floor rises under a loaded CPU). This mirrors the
    // shipped witness's own pre-measurement settle EXACTLY; it does NOT touch any frozen
    // r2 floor / slope band, and the median-based dimensions (D2..D5, D7, D8) do not need it.
    if (dim === 'D1' || dim === 'D6') {
 ...
 * child that writes partial/garbage/empty JSON is a testable, named failure mode
 * rather than an inline try/catch only exercised by a real spawn. Exported for the
 * QA gate (test/Bench.test.mjs).
 * @param {string} member
 * @param {string} dim
 * @param {string} stdout
 * @returns {object}
...
function printHeader(seed) {
    ...
    console.log('===========================================================================');
    console.log('@zakkster/lite-logn -- benchmark suite (' + DIMENSIONS.length + ' dimensions, ' +
        SUBJECTS.length + ' members vs O(n) foils)');
    console.log('===========================================================================');
    ...
    console.log('  NOTE:    D1 is the O(log n) WITNESS (r2/slope/foil-r2), delegated to test/witness.mjs.');
...
function summarize(results) {
    ...
    console.log('-- D1 O(log n) Witness (nsPerOp = intercept + slope*log2 n) -----------------');
    console.log('  member.op          r2       slope    band              on?   foil r2  off?');
    for (const m of SUBJECTS) {
        const r = results[m + '/D1'];
        for (const o of r.ops) {
            const band = '[' + fmt(o.slopeLo) + ',' + fmt(o.slopeHi) + ']';
            console.log('  ' + (m + '.' + o.op).padEnd(18) + ' ' +
                o.r2.toFixed(4).padStart(7) + '  ' + fmt(o.slope).padStart(6) + '  ' +
                band.padStart(15) + '  ' + (o.onLine ? 'ON ' : 'OFF') + '   ' +
                o.foilR2.toFixed(4).padStart(6) + '  ' + (o.foilOff ? 'off' : 'ON!'));
        }
        if (typeof r.counterFoil === 'object') {
            console.log('    counter-foil ' + r.counterFoil.name + ' get=' + fmt(r.counterFoil.getNsPerOp) +
                ' ns/op (O(1), order-blind); max single insert=' + fmt(r.maxSingleOp) + ' ns (disclosed)');
        }
    }
    console.log('');
    ...
    for (const m of SUBJECTS) {
        ...
        console.log('  ' + m.padEnd(12) + ' drift=' + fmt(r.drift));
    }
    console.log('');
    ...
    // D6 alloc + GC curve.
    console.log('-- D6 GC pressure (per op-row, max over the n=1e3..1e6 curve) --------------');
...
async function orchestrate() {
    ...
    // Drift sentinel: re-run ONE canonical cheap cell (BinaryHeap/D2) after the whole
    // matrix and compare its first-checkpoint ns/op to the first run's. A large gap
    // means the box heated up (thermal throttle / turbo down-clock) DURING the run, so
    // earlier cells were measured on a faster machine than later ones. This is
    // DISCLOSURE, not a hard failure: a noisy run is still a run, so exit stays 0.
    let sentinelKey = results['BinaryHeap/D2'] ? 'BinaryHeap/D2' : null;
    if (!sentinelKey) for (const m of SUBJECTS) if (results[m + '/D2']) { sentinelKey = m + '/D2'; break; }
    let sentinel = null;
    if (sentinelKey) {
        const first = sentinelMetric(results[sentinelKey]);
        const [sMember, sDim] = sentinelKey.split('/');
        const second = sentinelMetric(spawnCell(sMember, sDim, seed));
        const drift = driftFraction(first, second);
        sentinel = { cell: sentinelKey, first, second, drift, limit: DRIFT_LIMIT };
        if (driftExceeds(first, second)) {
            console.log('[bench] WARNING: thermal/turbo drift ' + (drift * 100).toFixed(0) +
                '% on the sentinel cell (' + sentinelKey + ' ' + fmt(first) + ' -> ' +
                fmt(second) + ' ns/op) -- results may be noisy');
        }
    }
...
// (e.g. `import { parseCellResult } from '../benchmark/Bench.mjs'`) would itself
// spawn the entire benchmark suite as a side effect of module evaluation -- an
// adversarial re-entrancy the QA pass caught.
