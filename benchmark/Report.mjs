/**
 * @zakkster/lite-logn -- benchmark report renderer (zero-dep, hand-rolled SVG).
 *
...
export function renderHtml(payload) {
    ...
    {
        const rowKeys = [];
        for (const m of subjects) for (const o of get(m, 'D1').ops) rowKeys.push(m + '.' + o.op);
        const slopes = [];
        for (const m of subjects) for (const o of get(m, 'D1').ops) slopes.push(o.slope);
        const chart = renderSvg({
            type: 'bar', title: 'D1 slope per op-row', xlabel: 'op-row', ylabel: 'ns/level (slope)',
            labels: rowKeys,
            series: [{ name: 'slope (ns/level)', color: color(0), values: slopes }],
        });
        const rows = [];
        for (const m of subjects) {
            for (const o of get(m, 'D1').ops) {
                rows.push([m + '.' + o.op, o.r2.toFixed(4), num(o.slope),
                    '[' + num(o.slopeLo) + ', ' + num(o.slopeHi) + ']', o.onLine ? 'ON-LINE' : 'off',
                    o.foilR2.toFixed(4), o.foilOff ? 'off-line' : 'ON!', o.foilName]);
            }
        }
        // SkipList counter-foil + disclosed max single insert.
        const sl = get('SkipList', 'D1');
        let note = 'The O(log n) Witness (delegated to test/witness.mjs): each op-row fits ' +
            'nsPerOp = intercept + slope*log2(n). ON-LINE = r2 >= 0.958 AND slope in the frozen ' +
            'per-op band; the O(n) foil must LEAVE the line (its r2 < 0.958).';
        if (sl && typeof sl.counterFoil === 'object') {
            note += ' SkipList counter-foil = a native Map: get ' + num(sl.counterFoil.getNsPerOp) +
                ' ns/op (O(1), FLATTER than any log line) but ORDER-BLIND (no ' +
                sl.counterFoil.cannotAnswer.join(' / ') + '). Disclosed MAX single insert = ' +
                num(sl.maxSingleOp) + ' ns (SkipList is EXPECTED O(log n), not gated).';
        }
        sections.push(section('D1 -- O(log n) Witness fit', note, chart,
            tableRows(['op-row', 'r2', 'slope', 'slope band', 'on line?', 'foil r2', 'foil off?', 'foil'], rows)));
    }
    ...
    // D3 -- memory.
    {
        ...
        sections.push(section('D3 -- Memory footprint + stability',
            ...
            'it RISES as load falls for BinaryHeap + SkipList (fixed backing over fewer live) and is FLAT for the ' +
            'INDEX-ADDRESSED Fenwick + SegmentTree (every cell always live).',
    ...
    // D4 -- cache proxy.
    {
        ...
        sections.push(section('D4 -- Cache behaviour (PROXY)',
            'PROXY ONLY: no native perf counters. Dense-iteration (forEach) ns/element rising with the working set is the cache-pressure proxy; dense (sequential) vs random single-element lookup gap.',
    ...
    // D6 -- GC pressure (per op-row).
    {
        const rows = [];
        for (const m of subjects) {
            const r = get(m, 'D6');
            for (const o of r.perOp) {
                const tputs = o.points.map((p) => num(p.opsPerMs)).join(' / ');
                rows.push([m + '.' + o.op, String(o.zeroAlloc), tputs]);
            }
        }
    ...
    // D7 -- scalability.
    {
        const rows = subjects.map((m) => {
            ...
            const io = typeof r.insertionOrder === 'object'
                ? (num(r.insertionOrder.sorted) + ' / ' + num(r.insertionOrder.random) + ' / ' + num(r.insertionOrder.adversarial))
                : String(r.insertionOrder);
        ...
        sections.push(section('D7 -- Scalability across key types + load factors + insertion order',
            'lite-logn members are numeric substrates: string + object keys read n/a (never 0). Load factors 0.3/0.5/0.7/0.9. Insertion order (sorted / random / adversarial-reverse) applies to the comparison-ordered BinaryHeap + SkipList; the index-addressed Fenwick + SegmentTree read n/a (positional, order-invariant).',
    ...
    // D8 -- workloads.
    {
        const rows = subjects.map((m) => {
            const r = get(m, 'D8');
            const ordered = typeof r.ordered === 'object'
                ? (num(r.ordered.successorNsPerOp) + ' succ / ' + num(r.ordered.rangeScanNsPerKey) + ' scan') : String(r.ordered);
            return [m, num(r.churn.nsPerOp), ordered];
        });
        sections.push(section('D8 -- Workload micro-benchmarks',
            'Churn (insert/delete or update the same keys -- all members) + ordered scan (successor + rangeIter -- SkipList only). Inapplicable workloads read n/a.',
    ...
    const head = '<header><h1>@zakkster/lite-logn -- benchmark report</h1>' +
        ...
        '<p class="note">Repo-only measurement infra. D1 is the O(log n) WITNESS (delegated to test/witness.mjs); ' +
        'every member is measured against the honest O(n) default a working programmer reaches for. ' +
