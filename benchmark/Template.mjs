/**
 * @zakkster/lite-logn -- package-agnostic benchmark TEMPLATE (the blueprint).
 *
 * Repo-only dev infra. This is a REUSABLE core a sibling package (lite-o1 with an
 * O(1) witness, lite-loglogn with an O(log log U) witness) can drop in: describe
 ...
 * reuses the SAME Harness primitives lite-logn's own suite uses, so adopting it is a
 * drop-in, NOT a risky rewrite of a working suite.
 *
 * ADOPTED from lite-o1 (decisions/0009 amendment 2): repo-only, NO new npm package.
 * lite-logn's own Matrix/Dimensions are a FULLER, hand-tuned expression of the same
 * ideas (8 dimensions, per-member foils, fail-closed dispatch, D1 = the O(log n)
 * Witness fit); this template is the portable SUBSET a sibling starts from and grows
 * the other 7 dimensions into at its own marked fill-in points.
 *
 ...
 *      of lite-logn/benchmark/Dimensions.mjs, which is the reference implementation).
 *
