...
import {
    OP_CLASS, OLOGN_WORST, OLOGN_EXPECTED, OLOGN_AMORTIZED, OLOGN2_WORST, CLEAR_WITNESS, CLEAR_WITNESS_EXCLUDED,
...
const OPS_PER_MEMBER = { BinaryHeap: 1, Fenwick: 2, SegmentTree: 2, SkipList: 2, Treap: 1, Scapegoat: 1, MinMaxHeap: 1, SplayTree: 1, BinomialHeap: 1, PairingHeap: 1, FibonacciHeap: 1, Fenwick2D: 2, SegmentTree2D: 2, SortedArray: 1, PersistentSegTree: 1, MergeSortTree: 1, WaveletTree: 1 };
...
for (const member of SUBJECTS) {
    ...
    test('D7: ' + member + ' string/object keys are the NA string; insertion order NA unless order-sensitive', () => {
        ...
        if (member === 'BinaryHeap' || member === 'SkipList' || member === 'Treap' || member === 'Scapegoat' || member === 'MinMaxHeap' || member === 'SplayTree' || member === 'BinomialHeap' || member === 'PairingHeap' || member === 'FibonacciHeap') {
    ...
    test('D8: ' + member + ' ordered workload is the NA string unless an ordered map (never 0)', () => {
        const r = D8(member, OPTS.D8);
        assert.ok(r.churn.nsPerOp > 0, member + ' churn must be positive');
        if (member === 'SkipList' || member === 'Treap' || member === 'Scapegoat' || member === 'SplayTree' || member === 'SortedArray') {
...
test('structure: D1 emits exactly the 22 gated witness op-rows across the 17 members', () => {
    let total = 0;
    const rowKeys = [];
    ...
    assert.equal(total, 22, 'D1 must emit exactly 22 gated op-rows');
    assert.deepEqual(rowKeys, OP_ROWS, 'the emitted op-rows must equal Matrix.OP_ROWS');
});

test('structure: the FOILS registry is the 22 gated rows + the SkipList counter-foil; cells() is 17 x 8 = 136', () => {
    ...
    assert.equal(Object.keys(FOILS).length, OP_ROWS.length + 1, 'FOILS = 22 gated rows + 1 counter-foil');
    assert.equal(cells().length, SUBJECTS.length * DIMENSIONS.length, 'matrix must be 17 x 8 = 136 cells');
...
test('D1 foil departure (all 22 gated op-rows): every foil genuinely MISSES the R^2 floor', () => {
    let checked = 0;
    for (const member of SUBJECTS) {
    ...
    assert.equal(checked, 22, 'must have checked all 22 gated op-rows for foil departure');
});
...
test('RATIONALE: all 11 members carry a FAIR-ALREADY verdict; only SkipList names a counter-foil', () => {
    for (const m of SUBJECTS) {
        const r = RATIONALE[m];
...
test('#1 witness gate UNCHANGED: R^2 floor 0.958, BinaryHeap band [5.76,13.44], MEMBERS.length 22', () => {
    ...
    assert.equal(WITNESS_MEMBERS.length, 22, 'the 22 gated witness op-rows (21 prior + WaveletTree.quantile)');
});
...
test('#2 OP_CLASS: covers the 22 witness rows; SkipList/Treap EXPECTED; Scapegoat get WORST + set/delete AMORTIZED; SplayTree get/set/delete AMORTIZED; MinMaxHeap WORST; BinomialHeap push AMORTIZED + popMin/meld WORST; PairingHeap + FibonacciHeap popMin/decreaseKey/remove AMORTIZED; MergeSortTree countLE O(log^2 n); WaveletTree quantile O(log n); no op painted O(1)', () => {
    ...
    for (const key of OP_ROWS) {
        ...
        assert.ok(/O\(log(\^2)? n\)/.test(OP_CLASS[key]), key + ' must be labelled O(log n) / O(log^2 n), got ' + OP_CLASS[key]);
    }
    ...
    for (const [key, cls] of Object.entries(OP_CLASS)) {
        ...
        assert.ok(cls === OLOGN_WORST || cls === OLOGN_EXPECTED || cls === OLOGN_AMORTIZED || cls === OLOGN2_WORST, key + ' unknown class ' + cls);
    }
    ...
    // SortedArray.get is WORST-case O(log n) too: a DETERMINISTIC contiguous lower-bound binary
    // search (no RNG, no amortization -- the Scapegoat.get analogue). Mislabelling it BITES.
    for (const op of ['BinaryHeap.push', 'BinaryHeap.pop', 'Fenwick.update', 'Fenwick.prefix', 'SegmentTree.update', 'SegmentTree.query', 'Scapegoat.get', 'MinMaxHeap.push', 'MinMaxHeap.popMin', 'MinMaxHeap.popMax', 'BinomialHeap.popMin', 'BinomialHeap.meld', 'SortedArray.get', 'PersistentSegTree.query', 'PersistentSegTree.update', 'WaveletTree.quantile']) {
    ...
    // BinomialHeap.push is O(1) AMORTIZED (a binary carry; a full carry is the O(log n) worst tail) --
    // DETERMINISTIC (no RNG), the same amortized honesty class. Mislabelling it worst-case or expected BITES.
    // PairingHeap.popMin / decreaseKey / remove are AMORTIZED O(log n) (a two-pass fold or cut-and-
    // link can be O(n), amortizing to O(log n)) -- DETERMINISTIC, no RNG. push and meld are strict
    // O(1), deliberately NOT in OP_CLASS (an O(1) op cannot carry an O(log n) label; the no-O(1)-token
    // rule below would reject one). Mislabelling any of the three as worst-case or expected BITES.
    ...
    // PairingHeap / FibonacciHeap push + meld are strict O(1) -- they must NOT appear in the O(log n)
    // honesty table at all (adding one would need an O(1) token, which the no-O(1) check forbids).
    assert.ok(!('PairingHeap.push' in OP_CLASS), 'PairingHeap.push is O(1); it must NOT be in the O(log n) OP_CLASS table');
    assert.ok(!('PairingHeap.meld' in OP_CLASS), 'PairingHeap.meld is O(1); it must NOT be in the O(log n) OP_CLASS table');
    assert.ok(!('FibonacciHeap.push' in OP_CLASS), 'FibonacciHeap.push is O(1); it must NOT be in the O(log n) OP_CLASS table');
    assert.ok(!('FibonacciHeap.meld' in OP_CLASS), 'FibonacciHeap.meld is O(1); it must NOT be in the O(log n) OP_CLASS table');
    // Fenwick2D.update / rectSum are WORST-case but O(log^2 n) (a range over TWO dimensions is a
    // squared log, never a single log) -- the distinct OLOGN2_WORST honesty class. Mislabelling
    // either as the single-log OLOGN_WORST (understating the second dimension's price) BITES.
    // MergeSortTree.countLE is ALSO O(log^2 n) WORST-case (O(log n) canonical nodes x O(log n) per-node
    // binary search) -- a STATIC immutable tree, no randomization / amortization. Same squared-log class.
    for (const op of ['Fenwick2D.update', 'Fenwick2D.rectSum', 'SegmentTree2D.update', 'SegmentTree2D.query', 'MergeSortTree.countLE']) {
        assert.equal(OP_CLASS[op], OLOGN2_WORST, op + ' must be the O(log^2 n) WORST-case class string');
        assert.ok(/O\(log\^2 n\)/.test(OP_CLASS[op]), op + ' must carry the squared-log token');
        assert.notEqual(OP_CLASS[op], OLOGN_WORST, op + ' must NOT be understated as single-log O(log n)');
    }
    // Full-teeth partition: every OP_CLASS key is accounted for by EXACTLY one of the four
...
test('#3 CLEAR_WITNESS is the fifteen MUTABLE SUBJECTS (static MergeSortTree + WaveletTree EXCLUDED); NodePool EXCLUDED; each has a real clear()', () => {
    // MergeSortTree and WaveletTree are SUBJECTS (17 total) but STATIC/IMMUTABLE -- no clear(), so
    // they are NOT clear-witnesses. The witness set is SUBJECTS minus those two static members.
    assert.deepEqual(CLEAR_WITNESS, SUBJECTS.filter((m) => m !== 'MergeSortTree' && m !== 'WaveletTree'),
        'the clear() witness set must be the mutable SUBJECTS (SUBJECTS minus MergeSortTree + WaveletTree)');
    ...
    assert.ok(typeof CLEAR_WITNESS_EXCLUDED.MergeSortTree === 'string' && CLEAR_WITNESS_EXCLUDED.MergeSortTree.length > 0,
        'MergeSortTree must be named in the EXCLUDED table with a reason (static/immutable, no clear())');
    assert.ok(typeof CLEAR_WITNESS_EXCLUDED.WaveletTree === 'string' && CLEAR_WITNESS_EXCLUDED.WaveletTree.length > 0,
        'WaveletTree must be named in the EXCLUDED table with a reason (static/immutable, no clear())');
    // Adding NodePool (the private free-list) to the set would break the equality above.
...
test('#3 retention: 1000 clear->refill cycles leave content 0 + free-list restored + no memberBytes growth', () => {
    for (const m of CLEAR_WITNESS) {
        ...
        for (let c = 0; c < 1000; c++) {
            ...
            else if (m === 'BinomialHeap') { assert.equal(obj.size, 0, m + ' size 0 after clear (cycle ' + c + ')'); for (let k = 0; k < 512; k++) obj.push(k, k); }
            else if (m === 'PairingHeap') { assert.equal(obj.size, 0, m + ' size 0 after clear (cycle ' + c + ')'); for (let k = 0; k < 512; k++) obj.push(k, k); }
            else if (m === 'FibonacciHeap') { assert.equal(obj.size, 0, m + ' size 0 after clear (cycle ' + c + ')'); for (let k = 0; k < 512; k++) obj.push(k, k); }
            ...
            else if (m === 'Fenwick2D') { assert.equal(obj.rectSum(0, 0, obj.rows - 1, obj.cols - 1), 0, m + ' accumulator 0 after clear'); const R = obj.rows, C = obj.cols; for (let r = 0; r < R; r++) for (let cc = 0; cc < C; cc++) obj.update(r, cc, 1); }
...
test('#6 shipping discipline: package.json.version is 1.1.0; benchmark/ stays repo-only', () => {
    const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'));
    assert.equal(pkg.version, '1.1.0', 'the 1.1.0 release: version is 1.1.0');
