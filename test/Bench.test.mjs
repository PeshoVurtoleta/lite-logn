...
const OPS_PER_MEMBER = { BinaryHeap: 1, Fenwick: 2, SegmentTree: 2, SkipList: 2, Treap: 1, Scapegoat: 1, MinMaxHeap: 1, SplayTree: 1, BinomialHeap: 1, PairingHeap: 1, FibonacciHeap: 1, Fenwick2D: 2 };
...
test('structure: D1 emits exactly the 16 gated witness op-rows across the 12 members', () => {
    let total = 0;
    const rowKeys = [];
    ...
    assert.equal(total, 16, 'D1 must emit exactly 16 gated op-rows');
    assert.deepEqual(rowKeys, OP_ROWS, 'the emitted op-rows must equal Matrix.OP_ROWS');
});

test('structure: the FOILS registry is the 16 gated rows + the SkipList counter-foil; cells() is 12 x 8 = 96', () => {
    ...
    assert.equal(Object.keys(FOILS).length, OP_ROWS.length + 1, 'FOILS = 16 gated rows + 1 counter-foil');
    assert.equal(cells().length, SUBJECTS.length * DIMENSIONS.length, 'matrix must be 12 x 8 = 96 cells');
...
test('D1 foil departure (all 16 gated op-rows): every foil genuinely MISSES the R^2 floor', () => {
    let checked = 0;
    for (const member of SUBJECTS) {
    ...
    assert.equal(checked, 16, 'must have checked all 16 gated op-rows for foil departure');
});
...
test('#1 witness gate UNCHANGED: R^2 floor 0.958, BinaryHeap band [5.76,13.44], MEMBERS.length 16', () => {
    ...
    assert.equal(WITNESS_MEMBERS.length, 16, 'the 16 gated witness op-rows (14 prior + Fenwick2D.update/rectSum)');
});
...
test('#2 OP_CLASS: covers the 14 witness rows; SkipList/Treap EXPECTED; Scapegoat get WORST + set/delete AMORTIZED; SplayTree get/set/delete AMORTIZED; MinMaxHeap WORST; BinomialHeap push AMORTIZED + popMin/meld WORST; PairingHeap + FibonacciHeap popMin/decreaseKey/remove AMORTIZED; no op painted O(1)', () => {
    ...
    for (const op of ['Fenwick2D.update', 'Fenwick2D.rectSum']) {
    ...
    const log2Ops = ['Fenwick2D.update', 'Fenwick2D.rectSum'];
...
test('#3 CLEAR_WITNESS is EXACTLY the twelve SUBJECTS; NodePool is EXCLUDED with a reason; each has a real clear()', () => {
    assert.deepEqual(CLEAR_WITNESS, SUBJECTS, 'the clear() witness set must be exactly SUBJECTS (12)');
    assert.equal(CLEAR_WITNESS.length, 12, 'exactly twelve members');
...
test('#3 clearWitness probe: the nine members reach size/content 0 + zero-alloc + reusable', () => {
...
test('#3 clearWitness is NON-VACUOUS: it calls each member\'s REAL prototype clear() exactly cycles+2 times', () => {
    const CLASSES = { BinaryHeap, Fenwick, SegmentTree, SkipList, Treap, Scapegoat, MinMaxHeap, SplayTree, BinomialHeap, PairingHeap, FibonacciHeap, Fenwick2D };
    ...
    try {
        ...
        assert.equal(cw.members.length, 12);
        for (const m of CLEAR_WITNESS) {
...
test('#6 shipping discipline: package.json.version is 0.12.0; benchmark/ stays repo-only', () => {
    const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'));
    assert.equal(pkg.version, '0.12.0', 'the Fenwick2D release: version is 0.12.0');
