...
There is deliberately NO subtree aggregate (path-only -- a subtree fold is the documented boundary of a future `EulerTourTree`), and NO `forEach` / `[Symbol.iterator]` -- the represented forest is walked through `findRoot` / `connected`, not enumerated. It is the family's first structure whose TOPOLOGY is mutable while the vertex set stays fixed -- the reason it allocates nothing per op (see the ADR).
