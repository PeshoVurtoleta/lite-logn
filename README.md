...
**v0.12.0 ships twelve members: BinaryHeap, Fenwick, SegmentTree, SkipList, Treap, Scapegoat, MinMaxHeap, SplayTree, BinomialHeap, PairingHeap, FibonacciHeap and Fenwick2D.** Members land one per session, each append-only so prior members stay byte-identical. The planned roster below fills in per release.
...
One member per session, each landing append-only (prior members stay byte-identical). At v0.12.0, BinaryHeap, Fenwick, SegmentTree, SkipList, Treap, Scapegoat, MinMaxHeap, SplayTree, BinomialHeap, PairingHeap, FibonacciHeap and Fenwick2D are shipped.
...
| --- | --- | --- | --- |
| `VERSION` | `string` | `'0.12.0'` | The package version. One of the three version sites (package.json / `LogN.js` `VERSION` const / `llms.txt`), kept in lockstep and enforced in review. |
