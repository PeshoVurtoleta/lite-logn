...
One member per session, each landing append-only (prior members stay byte-identical). At v1.0.0, BinaryHeap, Fenwick, SegmentTree, SkipList, Treap, Scapegoat, MinMaxHeap, SplayTree, BinomialHeap, PairingHeap, FibonacciHeap, Fenwick2D, SegmentTree2D, SortedArray, PersistentSegTree and MergeSortTree are shipped.
...
| --- | --- | --- | --- |
| `VERSION` | `string` | `'1.0.0'` | The package version. One of the three version sites (package.json / `LogN.js` `VERSION` const / `llms.txt`), kept in lockstep and enforced in review. |
